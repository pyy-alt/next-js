...
import { insertCustomerSchemaType } from '@/zod-schemas/customer';
...
export const saveCustomerAction = actionClient
  ...
  .action(async ({ parsedInput: customer }: { parsedInput: insertCustomerSchemaType }) => {
    ...
    // 测试三
    // const query = sql.raw("SELECT * FROM Zdb")
    ...
    if (customer.id === 0) {
      ...
      return { message: `Customer ID #${result[0].insertedId} updated successfully` };
    }
    ...
    const result = await db
      ...
      .returning({ updateId:customers.id })
      return { message: `Customer ID #${result[0].updateId} updated successfully` }
  });
